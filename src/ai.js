...
export async function callAI(env, systemPrompt, userPrompt, preferHighQuality = false, stageHint = 'default') {
  ...
  if (env.MISTRAL_API_KEY) {
    ...
    providers.push({ type: 'mistral', model: 'mistral-small-latest', key: env.MISTRAL_API_KEY, endpoint: MISTRAL_ENDPOINT });
...
export async function testProviders(env) {
  ...
  if (env.MISTRAL_API_KEY) {
    ...
    try {
      const resp = await callOpenAICompatible(
        MISTRAL_ENDPOINT, 'mistral-small-latest', env.MISTRAL_API_KEY,
        testSystem, testPrompt, 50
      );
      results.push({ provider: 'mistral', model: 'mistral-small-latest', ok: true, response: resp.trim().substring(0, 50), latency: Date.now() - start });
    } catch (err) {
      results.push({ provider: 'mistral', model: 'mistral-small-latest', ok: false, error: err.message.substring(0, 150), latency: Date.now() - start });
    }
  } else {
    results.push({ provider: 'mistral', model: 'mistral-small-latest', ok: false, error: 'MISTRAL_API_KEY non configurée' });
  }
