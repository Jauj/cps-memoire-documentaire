...
import {
  ...
  reextractClaims, listDocumentsNeedingReextraction,
} from './docmemory.js';
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // Supporte skip_crossref=true pour ingestion en masse rapide
      if (path === '/memory/ingest' && request.method === 'POST') {
        const body = await request.json();
        const result = await ingestDocument(env, body, ctx);
      ...
      // POST /memory/documents/:id/reextract — Ré-extraire les claims d'un document
      const reextractMatch = path.match(/^\/memory\/documents\/([a-z0-9_]+)\/reextract$/);
      if (reextractMatch && request.method === 'POST') {
        const result = await reextractClaims(env, reextractMatch[1]);
        const status = result.error ? 400 : 200;
        return new Response(JSON.stringify({ version: VERSION, ...result }), { status, headers: cors });
      }

      // GET /memory/reextract-pending — Documents nécessitant ré-extraction
      if (path === '/memory/reextract-pending' && request.method === 'GET') {
        const result = await listDocumentsNeedingReextraction(env);
        return new Response(JSON.stringify({ version: VERSION, ...result }), { headers: cors });
      }
      ...
      return new Response(JSON.stringify({
        ...
        routes: {
          ...
          'DELETE /memory/documents/:id': 'Supprimer un document',
          'POST /memory/documents/:id/reextract': 'Ré-extraire claims d\'un document',
          'GET /memory/reextract-pending': 'Documents nécessitant ré-extraction',
