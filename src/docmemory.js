...
export async function ingestDocument(env, {
  ...
  org_name = '',
  author = '',
  skip_crossref = true,  // false = cross-référence active (lent)
  async_claims = true,   // true = extraction claims en waitUntil (rapide)
}, ctx = null) {
  if (!dbAvailable(env)) {
  ...
  try {
    ...
    await env.DB.prepare(`
      UPDATE documents SET chunk_count = ?, status = 'chunked', updated_at = ? WHERE id = ?
    ...
    // 5. Extraire les claims via IA — ASYNCHRONE via waitUntil si ctx dispo
    const extractClaimsAsync = async () => {
      try {
        const extractionResult = await extractClaimsFromDocument(env, docId, content, doc_type, skip_crossref);
        ...
        console.log(`[DocMemory] Claims extraits pour ${docId}: ${finalClaims} claims via ${extractionResult.provider}`);
      } catch (err) {
        console.error(`[DocMemory] Extraction async échouée pour ${docId}: ${err.message}`);
        await env.DB.prepare(`
    ...
    if (async_claims && ctx && typeof ctx.waitUntil === 'function') {
      // Mode asynchrone : réponse immédiate, extraction en arrière-plan
      ctx.waitUntil(extractClaimsAsync());
      return {
        success: true,
        document_id: docId,
        title,
        ...
        claims_extracted: 0,
        status: 'chunked',
        provider: 'async_pending',
        note: 'Claims extraction en cours en arrière-plan',
      };
    } else {
...
export async function ingestFromURL(env, {
  ...
  try {
    ...
    // PDF : extraction via Jina AI Reader (gratuit)
    if (contentType.includes('application/pdf') || url.endsWith('.pdf')) {
      try {
        const jinaUrl = `https://r.jina.ai/${url}`;
        const pdfResp = await fetch(jinaUrl, {
        ...
        if (!pdfResp.ok) {
          return { error: `Extraction PDF échouée via Jina: HTTP ${pdfResp.status}`, retryable: pdfResp.status === 429 };
        }
        content = await pdfResp.text();
        content = content
          .replace(/^#+\s+/gm, '')
          .replace(/\*{1,2}([^*]+)\*{1,2}/g, '$1')
          .replace(/\[([^\]]+)\]\([^)]+\)/g, '$1')
        ...
        return { error: `Extraction PDF échouée: ${pdfErr.message}` };
      }
    } else if (contentType.includes('text/html') || contentType.includes('text/plain')) {
      // Essayer UTF-8 d'abord, puis Windows-1252 (fichiers Word anciens)
      let rawHtml;
      const buffer = await response.arrayBuffer();
      rawHtml = new TextDecoder('utf-8').decode(buffer);

      // Détecter le mojibake (caractères corrompus typiques du Windows-1252 lu en UTF-8)
      const hasMojibake = rawHtml.includes('\u00c2\u00a0')  // &nbsp; mojibake
        || (rawHtml.includes('\ufffd') && rawHtml.includes('Microsoft'))
        || /[\u00e2\u00c2\u00ef\u00cf][\u0080-\u00bf]/.test(rawHtml);

      if (hasMojibake) {
        try {
          rawHtml = new TextDecoder('windows-1252').decode(buffer);
        } catch (_) {
          // Garder la version UTF-8 si le décodage échoue
        }
      }

      // Extraire le titre AVANT nettoyage
      if (!title) {
        const titleMatch = rawHtml.match(/<title[^>]*>(.*?)<\/title>/i);
        title = titleMatch ? titleMatch[1].trim() : null;
      }
      // Nettoyer le HTML (Word HTML, métadonnées, etc.)
      content = rawHtml
        ...
        .replace(/<(nav|header|footer|menu|aside|form)[^>]*>[\s\S]*?<\/\1>/gi, '')
        .replace(/<[^>]+>/g, ' ')
      ...
      // Supprimer le préfixe de métadonnées Word (avant le contenu réel)
      const editorialMatch = content.match(/[ÉE]ditorial/);
      if (editorialMatch && editorialMatch.index > 200) {
        content = content.substring(editorialMatch.index);
      }
    } else {
    ...
    // Utiliser le nom de fichier ou l'URL comme titre par défaut
    if (!title) {
      const urlPath = url.split('/').pop() || '';
      title = urlPath.replace(/[_-]/g, ' ').replace(/\.(htm|html|pdf|php)/, '').trim() || 'Document sans titre';
    }
...
async function extractClaimsFromDocument(env, documentId, content, docType, skipCrossref = true) {
  try {
    // Tronquer le contenu pour rester dans les limites du contexte
    const maxChars = 15000; // ~4000 mots (limite pour extraction IA fiable)
    const truncatedContent = content.length > maxChars
      ? content.substring(0, maxChars) + '\n\n[... document tronqué pour le prompt IA — le texte COMPLET est dans les chunks ...]'
      : content;

    const userPrompt = `Voici un ${docType} à analyser. Extrais les assertions substantielles :\n\n---\n${truncatedContent}\n---\n\nProduis le JSON des claims.`;

    const result = await callAI(env, CLAIM_EXTRACTION_SYSTEM, userPrompt, false, 'drafting');
    let jsonStr = (typeof result === 'string' ? result : result.content || '').trim();
    ...
    // Réparer les erreurs JSON courantes de Mistral
    // 1. Virgule trailing avant } ou ]
    jsonStr = jsonStr.replace(/,\s*([}\]])/g, '$1');
    // 2. Guillemets non échappés dans les valeurs
    jsonStr = jsonStr.replace(/\\(?=[^"\\\/bfnrtu])/g, '');
    // 3. Propriété sans valeur (ex: "key": )
    ...
    try {
      ...
      // Dernier recours : extraire les claims individuellement
      console.warn(`[DocMemory] JSON parse échoué pour ${documentId}, tentative extraction partielle`);
      const claimRegex = /"claim_text"\s*:\s*"((?:[^"\\]|\\.)*)"/g;
      const partialClaims = [];
      let match;
      while ((match = claimRegex.exec(jsonStr)) !== null && partialClaims.length < 30) {
        partialClaims.push({ claim_text: match[1].replace(/\\"/g, '"').replace(/\\n/g, '\n'), claim_type: 'position', topic: 'sans_theme', stance: 'neutre', confidence: 0.3 });
      }
      if (partialClaims.length > 0) {
        parsed = { claims: partialClaims };
      } else {
        return { claims: [], error: `JSON parse: ${parseErr.message}`, provider: 'unknown' };
      }
    }
    ...
    // Cross-référence uniquement si demandé (désactivé par défaut pour ingestion en masse)
    if (!skipCrossref) {
      await crossReferenceNewClaims(env, claims, documentId);
    }

    const providerName = typeof result === 'object' ? (result.provider || 'ai') : 'ai';
    return { claims, provider: providerName, count: claims.length };
  } catch (err) {
...
/**
 * Ré-extrait les claims d'un document existant (chunks déjà en D1)
 * Utile pour les documents en erreur/chunked où l'extraction IA a échoué
 */
export async function reextractClaims(env, documentId) {
  if (!dbAvailable(env)) return { error: 'D1 non configuré' };
  ...
  try {
    ...
    // Récupérer le contenu complet depuis les chunks
    const { results: chunks } = await env.DB.prepare(
      'SELECT content FROM doc_chunks WHERE document_id = ? ORDER BY chunk_index'
    ).bind(documentId).all();

    if (!chunks || chunks.length === 0) {
      return { error: 'Aucun chunk trouvé pour ce document' };
    }

    const fullContent = chunks.map(c => c.content).join('\n\n');

    // Supprimer les anciennes claims et entrées liées
    const oldClaims = await env.DB.prepare(
      'SELECT id FROM claims WHERE document_id = ?'
    ).bind(documentId).all();

    if (oldClaims.results?.length > 0) {
      const claimIds = oldClaims.results.map(c => c.id);
      const placeholders = claimIds.map(() => '?').join(',');
      await env.DB.prepare(
        `DELETE FROM claim_relations WHERE from_claim_id IN (${placeholders}) OR to_claim_id IN (${placeholders})`
      ).bind(...claimIds, ...claimIds).run();
      await env.DB.prepare(
        `DELETE FROM topic_index WHERE claim_id IN (${placeholders})`
      ).bind(...claimIds).run();
      await env.DB.prepare(
        `DELETE FROM analysis_journal WHERE claim_id IN (${placeholders})`
      ).bind(...claimIds).run();
      await env.DB.prepare(
        'DELETE FROM claims WHERE document_id = ?'
      ).bind(documentId).run();
    }

    // Ré-extraire
    const extractionResult = await extractClaimsFromDocument(env, documentId, fullContent, doc.doc_type, true);
    const finalStatus = extractionResult.error ? 'error' : 'extracted';
    const finalClaims = extractionResult.claims?.length || 0;

    await env.DB.prepare(`
      UPDATE documents SET status = ?, claim_count = ?, updated_at = ? WHERE id = ?
    `).bind(finalStatus, finalClaims, now(), documentId).run();
...
/**
 * Liste les documents nécessitant une ré-extraction (error ou chunked avec 0 claims)
 */
export async function listDocumentsNeedingReextraction(env, limit = 50) {
  if (!dbAvailable(env)) return { documents: [], total: 0 };

  try {
    const { results } = await env.DB.prepare(`
      SELECT id, title, doc_date, status, claim_count, chunk_count, word_count
      FROM documents
      WHERE (status IN ('error', 'chunked') OR claim_count = 0)
    ...
    return { documents: results, total: countResult?.c || 0 };
  } catch (err) {
    return { documents: [], total: 0, error: err.message };
  }
}
